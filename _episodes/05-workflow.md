...
**Details on the Trimmomatic Parameters:**
	...
	..* ILLUMINACLIP:adapters.fasta:2:30:10 - Cut illumina-specific adapter sequences from the read.  Adapter sequences are given in the file adapters.fasta.  2 is the maximum number of mismatches to allow when searching for the adapters in the sequences (w/out affecting the match score).  30 is the palindrome clip threshold, which specifies the minimum match score for paired end adapters.  10 is the minimum match score for adapters in single end reads.
